...
- **Themes** — the four Carbon themes **White, Gray 10, Gray 90, Gray 100**
  (`@carbon/themes`), as `CarbonThemeData`: the core semantic tokens
  (`background`, `layer`, `text*`, `field*`, `border*`, `support*`, `focus`,
  `icon*`, …) mapped onto color constants per theme. Generated by
  `tool/generate_carbon_themes.py` and value-locked by an exhaustive test.
  Syntax-highlighting, AI-experimental, and chat token groups are deferred
  until the components that consume them are built.
